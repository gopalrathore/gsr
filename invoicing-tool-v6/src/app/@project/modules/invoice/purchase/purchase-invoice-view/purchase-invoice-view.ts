...
export class SalesInvoiceView {
  ...
  protected purchase_id = this.transfereService.getData();
  ...
  constructor(protected transfereService:TransfereService, protected router:Router){
    if (this.purchase_id) {
      ...
      this.router.navigateByUrl('/invoice/purchase');
    }
   ...
   */
  getInvoiceData():void{
    AsyncApiCall.get('purchaseInvoice', {purchase_id: this.purchase_id}).subscribe(resp => {
      console.log(JSON.stringify(resp.data[0], undefined,2));
